...
def voc_ap(rec, prec):
    ...
    for i in range(1, len(mrec)):
        if mrec[i] != mrec[i-1]: # changing mrec to mprec would be correct. but nothings will be changed.
...
def read_test_file(root):
    ...
    for fpath in folder_list:
        ...
        labels = elemTree.parse(filename[0]) # One .xml file in each folder.
        ...
        for label in labels :
            ...
            for polygon_info in label.findall('./polygon') :
                ...
                # count that object
                if class_name in gt_counter_per_class: # Can use defalut dict
